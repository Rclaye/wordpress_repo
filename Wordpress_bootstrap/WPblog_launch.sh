...
echo "Retrieving parameters from SSM Parameter Store in region ${AWS_REGION}..."

# Retrieve parameters from SSM Parameter Store
MYSQL_ROOT_PASSWORD=$(aws ssm get-parameter --name "/wordpress/mysql/rootpassword" --with-decryption --region ${AWS_REGION} --query "Parameter.Value" --output text 2>/dev/null || echo "")
WP_DB_NAME=$(aws ssm get-parameter --name "/wordpress/rds/dbname" --region ${AWS_REGION} --query "Parameter.Value" --output text 2>/dev/null || echo "wordpressdb")
WP_DB_USER=$(aws ssm get-parameter --name "/wordpress/db/username" --with-decryption --region ${AWS_REGION} --query "Parameter.Value" --output text 2>/dev/null || echo "")
WP_DB_PASSWORD=$(aws ssm get-parameter --name "/wordpress/db/password" --with-decryption --region ${AWS_REGION} --query "Parameter.Value" --output text 2>/dev/null || echo "")
WP_ADMIN_USER=$(aws ssm get-parameter --name "/wordpress/admin/username" --with-decryption --region ${AWS_REGION} --query "Parameter.Value" --output text 2>/dev/null || echo "")
WP_ADMIN_PASSWORD=$(aws ssm get-parameter --name "/wordpress/admin/password" --with-decryption --region ${AWS_REGION} --query "Parameter.Value" --output text 2>/dev/null || echo "")
WP_ADMIN_EMAIL=$(aws ssm get-parameter --name "/wordpress/admin/email" --with-decryption --region ${AWS_REGION} --query "Parameter.Value" --output text 2>/dev/null || echo "")

# Check if parameters were retrieved successfully, otherwise exit with error
if [ -z "$MYSQL_ROOT_PASSWORD" ] || [ -z "$WP_DB_USER" ] || [ -z "$WP_DB_PASSWORD" ] || [ -z "$WP_ADMIN_USER" ] || [ -z "$WP_ADMIN_PASSWORD" ] || [ -z "$WP_ADMIN_EMAIL" ]; then
    echo "Error: Failed to retrieve one or more required parameters from SSM Parameter Store."
    echo "Make sure the instance has an IAM role with permissions to access SSM parameters."
    exit 1
fi
